...
/* ---------------------------------------------------------
   CoyToy — Admin Login ("Access Terminal")
   Shares tokens with Home / Navbar:
...
const STYLE_ID = "coytoy-adminlogin-styles";
...
function ensureStylesInjected() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
@keyframes coytoy-twinkle { 0%, 100% { opacity: 0.25; } 50% { opacity: 1; } }
@keyframes coytoy-fade-up { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: translateY(0); } }
...
@keyframes coytoy-spin { to { transform: rotate(360deg); } }

.coytoy-login-card { animation: coytoy-fade-up 0.5s ease both; }
.coytoy-login-wordmark { animation: coytoy-flicker-in 1.2s ease-out both; }
.coytoy-login-error { animation: coytoy-shake 0.4s ease; }
...
.coytoy-field-input::placeholder { color: #5b6390; }
.coytoy-field-input:focus {
  outline: none;
  border-color: #3fe3ff;
  box-shadow: 0 0 0 3px rgba(63,227,255,0.18), 0 0 18px rgba(63,227,255,0.25);
}
.coytoy-field-input:focus-visible { outline: none; }
...
.coytoy-pw-toggle:hover { color: #3fe3ff; }
.coytoy-pw-toggle:focus-visible {
...
.coytoy-submit-btn:hover:not(:disabled) {
  background: rgba(255,63,199,0.22);
  box-shadow: 0 0 18px rgba(255,63,199,0.4);
}
.coytoy-submit-btn:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
.coytoy-submit-btn:focus-visible {
...
.coytoy-spinner {
  ...
  height: 14px;
  border-radius: 50%;
  border: 2px solid rgba(255,63,199,0.3);
  border-top-color: #ff3fc7;
  animation: coytoy-spin 0.7s linear infinite;
}

@media (prefers-reduced-motion: reduce) {
  .coytoy-login-card, .coytoy-login-wordmark, .coytoy-login-error, .coytoy-spinner { animation: none !important; }
}
@media (max-width: 480px) {
  .coytoy-login-card { padding: 28px 22px !important; }
}
`;
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: "8px" }}>
          ...
          <h1
            className="coytoy-login-wordmark"
            style={{
        ...
        <form onSubmit={handleLogin} noValidate>
          <div style={{ marginBottom: "16px" }}>
            ...
            <input
              ...
              placeholder="you@coytoybd.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="coytoy-field-input"
              required
            />
          </div>
          ...
          <button type="submit" className="coytoy-submit-btn" disabled={submitting}>
            {submitting && <span className="coytoy-spinner" aria-hidden="true" />}
