...
/* ---------------------------------------------------------
   CoyToy — Customer Signup
   Shares tokens + injected styles with Login (same STYLE_ID guard,
   so whichever page mounts first injects the CSS once).
--------------------------------------------------------- */
...
export default function Signup() {
  ...
  // If someone lands on /signup while already signed in, send them on.
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    if (!/^01[3-9][0-9]{8}$/.test(phone)) {
      return "Enter a valid 11-digit mobile number starting with 01.";
    }
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      try {
        ...
        // The auth account exists but the profile doc didn't save - don't
        // leave a half-created account behind, so login doesn't just fail
        // silently later on.
